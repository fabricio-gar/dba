...
public class DocumentacaoService {
	...
	public Aviso adicionaDocumento(DocumentacaoDTO documentacaoDTO) {
		try {
			...
			if(validar.isNull(documentacaoDTO)) return new Aviso(400, "Documento em branco");
			
			if(!validar.isValidCpfCnpj(documentacaoDTO.getCpfCnpjCliente())) return new Aviso(400, "Cpf ou Cnpj inválido");
